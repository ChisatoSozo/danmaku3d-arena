...
const ReactBabylonjsEngine: React.FC<EngineProps> = (
  props: EngineProps,
  context?: any
) => {
  ...
  const onBeforeRenderLoopObservable = useRef<Observable<Engine>>(
    new Observable<Engine>()
  );
  const onEndRenderLoopObservable = useRef<Observable<Engine>>(
    new Observable<Engine>()
  );
  ...
  useEffect(() => {
    ...
    const context = canvasRef.current.getContext("webgl2", { stencil: true });
    ...
    Engine.audioEngine = Engine.AudioEngineFactory(
      engine.current.getRenderingCanvas(),
      null,
      null
    );

    engine.current.renderEvenInBackground = true;
  ...
  return (
    <EngineCanvasContext.Provider
      value={{ engine: engine.current, canvas: canvasRef.current }}
    >
      <canvas
        {...opts}
        {...canvasProps}
