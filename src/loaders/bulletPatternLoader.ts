...
import {
  ...
  GLSLAssetDefinition,
} from "../types/gameDefinition/AssetDefinition";
import {
...
export const hashBulletPattern = (
  bulletPatternAssetDefinition: BulletPatternAssetDefinition
) => {
  const newBulletPatternAssetDefinition = deepExcludeKeyFromObject(
    bulletPatternAssetDefinition,
    "hash"
  );
...
export const bulletPatternLoaded = async (
  ...
  if (assets.bulletPatterns[hash]) {
    ...
    const patternDefinition = (await fetch(URI).then((response) =>
      response.json()
    )) as BulletPatternDefinition;
...
const glslAssetDefinitionToContent = (
  glslAssetDefinition: GLSLAssetDefinition,
  assets: Assets
) => {
  ...
  if (!glslAsset)
    throw new Error("glslAssetDefinitionToContent: glslAsset is undefined");

  const shaderContent = Effect.ShadersStore[glslAsset.shader + "PixelShader"];
  if (!shaderContent)
    throw new Error("glslAssetDefinitionToContent: shaderContent is undefined");
  return shaderContent;
};

const makeEnemyBulletPatternGLSL = (
  patternDefinition: EnemyBulletPatternDefinition,
  assets: Assets
) => {
  const positionPhases = patternDefinition.phases.map((phase): BulletPhase => {
    const at = phase.at;
    const positionInitializationGLSL = glslAssetDefinitionToContent(
      phase.positionInitializationGLSL,
      assets
    );
    const positionUpdateGLSL = glslAssetDefinitionToContent(
      phase.positionUpdateGLSL,
      assets
    );
  ...
  const velocityPhases = patternDefinition.phases.map((phase): BulletPhase => {
    const at = phase.at;
    const velocityInitializationGLSL = glslAssetDefinitionToContent(
      phase.velocityInitializationGLSL,
      assets
    );
    const velocityUpdateGLSL = glslAssetDefinitionToContent(
      phase.velocityUpdateGLSL,
      assets
    );
...
const makePlayerBulletPatternGLSL = (
  patternDefinition: PlayerBulletPatternDefinition,
  assets: Assets
) => {
  const positionUpdateGLSL = glslAssetDefinitionToContent(
    patternDefinition.positionUpdateGLSL,
    assets
  );
  const velocityUpdateGLSL = glslAssetDefinitionToContent(
    patternDefinition.velocityUpdateGLSL,
    assets
  );

  const positionFunctionGLSL = constructPlayerPixelShader(
    positionUpdateGLSL,
    "position"
  );
  const velocityFunctionGLSL = constructPlayerPixelShader(
    velocityUpdateGLSL,
    "velocity"
  );
...
const makeCollisionGLSL = (
  patternDefinition: BulletPatternDefinition,
  assets: Assets
) => {
  const collisionUpdateGLSL = glslAssetDefinitionToContent(
    patternDefinition._collisionFunctionGLSL,
    assets
  );
...
export const loadBulletPattern = async (
  ...
  const positionFunctionGLSLName =
    hashString(positionFunctionGLSL) + "PositionFunction";
  const velocityFunctionGLSLName =
    hashString(velocityFunctionGLSL) + "VelocityFunction";
  const collisionFunctionGLSLName =
    hashString(collisionFunctionGLSL) + "CollisionFunction";

  const positionFunctionGLSLHash = manualLoadGLSL(
    positionFunctionGLSLName,
    positionFunctionGLSL,
    "pixel",
    assets
  );
  const velocityFunctionGLSLHash = manualLoadGLSL(
    velocityFunctionGLSLName,
    velocityFunctionGLSL,
    "pixel",
    assets
  );
  const collisionFunctionGLSLHash = manualLoadGLSL(
    collisionFunctionGLSLName,
...
export const useBulletPatternAsset = (
  assetDefinition: BulletPatternAssetDefinition
) => {
...
export const useBulletPatternAssetArray = (
  assetDefinitions: BulletPatternAssetDefinition[]
) => {
  const assets = useAssets();
  return assetDefinitions.map((assetDefinition) =>
    getAsset(assets, assetDefinition)
  ) as BulletPatternAsset[];
};
