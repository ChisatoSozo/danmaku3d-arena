...
export const getAsset = (
  assets: Assets,
  assetDefinition: AnyAssetDefinition
) => {
  ...
  if (assetDefinition.type === "sound") {
    ...
    if (!assets.textures[hash])
      throw new Error(`Texture ${hash} was never loaded`);
    return assets.textures[hash];
  } else if (assetDefinition.type === "vector") {
    if (!assets.textures[hash])
      throw new Error(`Vector ${hash} was never loaded`);
    return assets.textures[hash];
  } else if (assetDefinition.type === "timing") {
    if (!assets.textures[hash])
      throw new Error(`Timing ${hash} was never loaded`);
    return assets.textures[hash];
  } else if (assetDefinition.type === "bulletPattern") {
    if (!assets.bulletPatterns[hash])
      throw new Error(`Bullet Pattern ${hash} was never loaded`);
    return assets.bulletPatterns[hash];
  } else if (assetDefinition.type === "terrain") {
    if (!assets.terrains[hash])
      throw new Error(`terrain ${hash} was never loaded`);
    return assets.terrains[hash];
  } else {
